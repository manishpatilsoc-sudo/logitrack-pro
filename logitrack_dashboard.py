...
<div style="text-align:center;padding-bottom:22px;margin-bottom:20px;border-bottom:1px solid rgba(129,140,248,0.07);">
  ...
  <div style="font-weight:900;font-size:18px;color:#c7d2fe;letter-spacing:-0.03em;">LogiTrack</div>
...
if page == "overview":
    ...
    with c1:
        ...
        show(fig, 310, "Monthly Orders Trend", "📅", "x unified")
...
# ═══ DELIVERY ═══
    ...
    with c2:
        ...
        fig.add_trace(go.Bar(x=q2["Month"], y=q2["Total"], name="Total",
            marker_color="rgba(129,140,248,0.3)", marker_cornerradius=5,
        ...
        fig.update_layout(**base_layout(290,"Monthly Orders vs Delays","📅","x unified"), barmode="overlay")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar":False})
    ...
    with c4:
        ...
        show(fig, 290, "Order Status by Category", "📊", "x unified")
...
# ═══ WAREHOUSE ═══
    ...
    with c1:
        ...
        fig.add_trace(go.Bar(name="Total", x=q["WH"], y=q["Orders"],
            marker_color="rgba(129,140,248,0.3)", marker_cornerradius=6,
            hovertemplate="Total: <b>%{y}</b><extra></extra>"))
        fig.add_trace(go.Bar(name="Delayed", x=q["WH"], y=q["Delayed"],
            marker_color="#f43f5e", marker_cornerradius=6,
            hovertemplate="Delayed: <b>%{y}</b><extra></extra>"))
        fig.update_layout(**base_layout(290,"Orders vs Delays by Warehouse","🏭","x unified"), barmode="group")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar":False})
    ...
    with c3:
        ...
        fig.update_traces(marker_line_width=0,
            hovertemplate="%{fullData.name}: <b>%{y}</b><extra></extra>")
        show(fig, 290, "Category Mix by Warehouse", "📁", "x unified")
    ...
    with c4:
        ...
        fig.update_traces(marker_line_width=0,
            hovertemplate="%{fullData.name}: <b>%{y}</b><extra></extra>")
        show(fig, 290, "Shipping Mode by Warehouse", "🚚", "x unified")
