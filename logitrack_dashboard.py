...
[data-testid="stPillsOption"]{
  font-size:8px!important;padding:1px 6px!important;
  min-height:0!important;height:auto!important;line-height:1.3!important;
  border-radius:20px!important;border:1px solid rgba(129,140,248,0.2)!important;
  background:transparent!important;color:#64748b!important;cursor:pointer!important;}
[data-testid="stPillsOption"]:hover{
  border-color:rgba(129,140,248,0.5)!important;color:#a5b4fc!important;}
...
<div style="margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid rgba(129,140,248,0.06);">
  <div style="display:flex;align-items:center;gap:11px;">
    ...
    <div style="font-size:24px;font-weight:900;letter-spacing:-0.025em;color:#e2e8f0;line-height:1.2;">{title}</div>
  </div>
...
if page == "overview":
    ...
    with c1:
        mdf = pd.read_sql("""
            SELECT Month, MonthSort, COUNT(*) AS Orders
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=5, color="#818cf8", line=dict(width=2, color="#0a0a1f")),
    ...
    with c3:
        cdf = pd.read_sql("""
            SELECT Category, ROUND(SUM(Final_Cost_INR)/1000,1) AS Rev_K
    ...
    with c4:
        mrev = pd.read_sql("""
            SELECT Month, MonthSort, ROUND(SUM(Final_Cost_INR)/1000,1) AS Rev_K
            FROM sc GROUP BY Month, MonthSort ORDER BY MonthSort""", con)
        fig = go.Figure()
        fig.add_trace(go.Scatter(
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c3:
        q3 = pd.read_sql("""
            SELECT Category, ROUND(AVG(Lead_Time_Days),1) AS AvgLT
            FROM sc GROUP BY Category ORDER BY AvgLT""", con)
    ...
    with c4:
        q4 = pd.read_sql("""
            ...
            FROM sc GROUP BY Category, Order_Status""", con)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c3:
        q3 = pd.read_sql("""
            SELECT Shipping_Mode, ROUND(AVG(Freight_Cost_INR),0) AS AvgF
            FROM sc GROUP BY Shipping_Mode ORDER BY AvgF DESC""", con)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        q = pd.read_sql("""
            SELECT Supplier_Name, ROUND(SUM(Final_Cost_INR)/1000,1) AS Rev_K
    ...
    with c2:
        q2 = pd.read_sql("""
            SELECT Supplier_Name, ROUND(AVG(Quality_Rating),2) AS AvgQ
    ...
    with c3:
        q3 = pd.read_sql("""
            SELECT Supplier_City, COUNT(*) AS Orders
            FROM sc GROUP BY Supplier_City ORDER BY Orders DESC""", con)
    ...
    with c4:
        q4 = pd.read_sql("""
            SELECT Supplier_Name, ROUND(AVG(Lead_Time_Days),1) AS AvgLT
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    q = pd.read_sql("""
        SELECT REPLACE(Warehouse,'WH-','') AS WH,
        COUNT(*) AS Orders,
        SUM(CASE WHEN Order_Status='Delayed' THEN 1 ELSE 0 END) AS Delayed
    ...
    with c2:
        q3 = pd.read_sql("""
            SELECT REPLACE(Warehouse,'WH-','') AS WH, Category, COUNT(*) AS Count
            FROM sc GROUP BY Warehouse, Category""", con)
